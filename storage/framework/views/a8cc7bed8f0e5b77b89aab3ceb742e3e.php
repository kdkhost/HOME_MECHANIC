        ...
        <form method="GET" class="row g-2 align-items-end">
            ...
            <div class="col-md-3">
                <select name="status" class="form-control form-control-sm">
                    ...
                    <option value="draft"     <?php echo e(request('status') === 'draft'     ? 'selected' : ''); ?>>Rascunhos</option>
                    <option value="archived"  <?php echo e(request('status') === 'archived'  ? 'selected' : ''); ?>>Arquivados</option>
                </select>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-2">
                <a href="<?php echo e(route('admin.blog.index')); ?>" class="btn btn-secondary btn-sm w-100"><i class="fas fa-times"></i> Limpar</a>
            </div>
        </form>
    </div>
</div>
...
<div class="card">
    <div class="card-header">
        ...
        <div class="card-tools">
            <span style="font-size:0.78rem;color:rgba(255,255,255,0.7);">
                <?php echo e(method_exists($posts, 'total') ? $posts->total() : count($posts)); ?> post(s)
            </span>
        </div>
    </div>
    <div class="card-body p-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if($isObj && $post->excerpt): ?>
                                <div class="text-muted" style="font-size:0.78rem;"><?php echo e(Str::limit($post->excerpt, 60)); ?></div>
                            <?php endif; ?>
                        </td>
                        <td>
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                <button type="button" class="btn btn-info" title="Ver" onclick="showPostModal(<?php echo e($id); ?>, '<?php echo e(addslashes($title)); ?>', '<?php echo e(addslashes($author)); ?>', '<?php echo e($date->format('d/m/Y')); ?>', '<?php echo e($status); ?>')">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <a href="<?php echo e(route('admin.blog.edit', $id)); ?>" class="btn btn-warning" title="Editar"><i class="fas fa-pencil-alt"></i></a>
                                <form method="POST" action="<?php echo e(route('admin.blog.destroy', $id)); ?>" style="display:inline;">
        ...
        <div class="px-3 py-2"><?php echo e($posts->withQueryString()->links()); ?></div>
        <?php endif; ?>
        <?php else: ?>
        <div class="empty-state">
            ...
            <a href="<?php echo e(route('admin.blog.create')); ?>" class="btn btn-primary"><i class="fas fa-plus"></i> Criar Post</a>
        </div>
        <?php endif; ?>
    </div>
</div>
...
<script>
const postsData = <?php echo json_encode($posts->map(fn($p) => [
    'id' => is_object($p) ? $p->id : $p['id'], 'content' => is_object($p) ? $p->content : $p['content'], 'cover_image' => is_object($p) ? $p->cover_image : ($p['cover_image'] ?? null)
])->keyBy('id')) ?>;
...
function showPostModal(id, title, author, date, status) {
    ...
    document.getElementById('modalEditLink').href = '<?php echo e(url("admin/blog")); ?>/' + id + '/edit';
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH G:\Tudo\HOME_MECHANIC\resources\views\modules\blog\index.blade.php ENDPATH**/ ?>