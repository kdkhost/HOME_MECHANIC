...
function loadServices(page) {
    ...
    $.ajax({
        url: '<?php echo e(route("admin.services.index")); ?>?' + params.toString(),
    ...
    if (list) {
        sortable = Sortable.create(list, {
            ...
            onEnd: function() {
                ...
                $.ajax({
                    url: '<?php echo e(route("admin.services.reorder")); ?>',
                    method: 'POST',
...
function editService(id) {
    $.ajax({
        url: '<?php echo e(route("admin.services.index")); ?>/' + id,
...
document.getElementById('svcForm').addEventListener('submit', function(e) {
    ...
    var url    = id
        ? '<?php echo e(route("admin.services.index")); ?>/' + id
...
function toggleActive(id) {
    $.ajax({
        url: '<?php echo e(route("admin.services.index")); ?>/' + id + '/toggle-active',
        method: 'PATCH',
...
function toggleFeatured(id) {
    $.ajax({
        url: '<?php echo e(route("admin.services.index")); ?>/' + id + '/toggle-featured',
        method: 'PATCH',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'), 'Accept': 'application/json' },
        success: function(d) { if (d.success) { HMToast.success(d.message); loadServices(currentPage); } else HMToast.error(d.message); },
        error: function() { HMToast.error('Erro de conexão.'); }
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH G:\Tudo\HOME_MECHANIC\resources\views\modules\services\index.blade.php ENDPATH**/ ?>