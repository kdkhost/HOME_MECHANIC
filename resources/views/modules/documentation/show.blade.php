...
@section('title', $title . ' - Documentação')
@section('page-title', $title)

@section('breadcrumb')
<li class="breadcrumb-item">
    <a href="{{ route('admin.documentation.index') }}">Documentação</a>
</li>
    ...
    <div class="col-lg-9">
        ...
        <div class="doc-meta no-print">
            <div class="row align-items-center">
                <div class="col-md-8">
                    ...
                    <strong>Última atualização:</strong> {{ date('d/m/Y H:i', $lastModified) }}
                </div>
                <div class="col-md-4 text-end">
                    <button onclick="window.print()" class="print-button me-2">
                        <i class="bi bi-printer"></i> Imprimir
                    </button>
                    <a href="{{ route('admin.documentation.index') }}" class="btn btn-outline-secondary btn-sm">
        ...
        <div class="doc-navigation no-print">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <a href="{{ route('admin.documentation.show', $navigation['previous']['slug']) }}" class="doc-nav-link">
                        <div class="text-muted small">← Anterior</div>
                        <div class="fw-bold">{{ $navigation['previous']['title'] }}</div>
                    </a>
                </div>
                @endif
                ...
                @if($navigation['next'])
                <div class="col-md-6">
                    <a href="{{ route('admin.documentation.show', $navigation['next']['slug']) }}" class="doc-nav-link text-end">
                        <div class="text-muted small">Próximo →</div>
                        <div class="fw-bold">{{ $navigation['next']['title'] }}</div>
                    </a>
                </div>
                @endif
            </div>
            ...
            <div class="card-body text-center">
                ...
                <div class="mt-2">
                    <small class="text-muted">
                        Encontrou um erro? 
                        <a href="mailto:suporte@homemechanic.com.br?subject=Erro na Documentação: {{ $title }}">
                            Reporte aqui
                        </a>
                    </small>
                </div>
...
$(document).keydown(function(e) {
    ...
    if (e.altKey && e.keyCode === 37) {
        @if($navigation['previous'])
        window.location.href = '{{ route("admin.documentation.show", $navigation["previous"]["slug"]) }}';
        @endif
    }
    ...
    if (e.altKey && e.keyCode === 39) {
        @if($navigation['next'])
        window.location.href = '{{ route("admin.documentation.show", $navigation["next"]["slug"]) }}';
        @endif
    }
