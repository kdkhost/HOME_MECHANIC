            ...
            <div class="installer-body">
                ...
                <div class="status-summary">
                    <div class="summary-item">
                        <div class="summary-number text-success">{{ collect($requirements['extensions'])->where('status', true)->count() }}</div>
                        <div class="summary-label">Extensões OK</div>
                    </div>
                    <div class="summary-item">
                        <div class="summary-number {{ $requirements['php_version']['status'] ? 'text-success' : 'text-danger' }}">
                            {{ $requirements['php_version']['status'] ? '✓' : '✗' }}
                        </div>
                        <div class="summary-label">PHP Version</div>
                    </div>
                    <div class="summary-item">
                        <div class="summary-number {{ isset($requirements['permissions']) && collect($requirements['permissions'])->where('status', true)->count() === count($requirements['permissions']) ? 'text-success' : 'text-danger' }}">
                            {{ isset($requirements['permissions']) ? collect($requirements['permissions'])->where('status', true)->count() : 0 }}
                        </div>
