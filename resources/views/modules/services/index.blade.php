...
function renderServices(services) {
    ...
    services.forEach(function(s) {
        ...
        html += '<div class="col-md-4 col-lg-3" data-id="' + s.id + '">' +
            ...
            '<button class="btn btn-danger btn-delete" onclick="event.stopPropagation(); event.preventDefault(); deleteService(' + s.id + ', \'' + (s.title || '').replace(/'/g,'') + '\'); return false;" title="Excluir"><i class="fas fa-trash"></i></button>' +
            '</div></div></div></div>';
    });
    html += '</div>';
    c.innerHTML = html;
